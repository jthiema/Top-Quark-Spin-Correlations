...
# Borrowed from mselvaggi, implemented other varaibles relevant to analysis
...
class TreeProducer:
    def __init__(self, debug):
         ...
         self.elec_sum_pt      = array( 'f', self.maxn*[ 0. ] )
         ...
         self.t.Branch( "elec_sum_pt", self.elec_sum_pt , "elec_sum_pt[elec_size]/I")
         ...
         self.t.Branch( "scalar_ht_size", self.weight_size , "scalar_ht_size/I")
         self.t.Branch( "scalar_ht"     , self.weight      , "scalar_ht[weight_size]/F")
    ...
    def processJets(self, jets):
        i = 0
        for item in jets:
            self.jet_pt      [i] = item.PT
            self.jet_eta     [i] = item.Eta
            self.jet_phi     [i] = item.Phi
            self.jet_mass    [i] = item.P4().M()
