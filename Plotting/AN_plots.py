...
def main():
    ...
    llbar_deta    = np.loadtxt('Delphes_llbar_dphi_iso.txt')
    tt_llbar_deta = np.loadtxt('Run2_FS_llbar_dphi_iso.txt')

    Delphes = np.histogram(llbar_deta   , bins=nbins)[0]
    Run2    = np.histogram(tt_llbar_deta, bins=nbins)[0]
    ...
    plotPad.DrawFrame(xlow, 0, xhigh, 0.15, ";\Delta \phi_{ll}; Arbitrary Units")
