...
mstop_mass_points = [175, 182.5, 190, 197.5,205, 212.5, 220, 227.5, 235, 242.5]
    ...
    for mc in mchi0_mass_points:
        if (ms - mc == 167.5) or (ms - mc == 175) or (ms - mc == 182.5):
            ...
            if '.' in str(ms) and '.' in str(mc):
                ip_file_name = 'Top_reco_op_Mstop_' + \
                    (str(ms).replace('.', '_')) + '_mchi_' + (str(mc).replace('.', '_')) + '.root'

            elif '.' in str(ms) and '.' not in str(mc):
                ip_file_name = 'Top_reco_op_Mstop_' + (str(ms).replace('.', '_')) + '_mchi_' + str(mc) + '.root'

            elif '.' not in str(ms) and '.' in str(mc):
                ip_file_name = 'Top_reco_op_Mstop_' + str(ms) + '_mchi_'  + (str(mc).replace('.', '_')) + '.root'

            else:
                ip_file_name = 'Top_reco_op_Mstop_' + str(ms) + '_mchi_'  + str(mc) + '.root'


            # Input files and trees
            print('Loading in the root file')
            try :
                ptr = uproot.open('Root_files/SUSY_top_reco_op/' + str(ip_file_name))['Step8']

            except : 
                print('File not found ::' + str(ip_file_name))
                print('Moving to the next mass point')
                continue
            ...
            if '.' in str(ms) and '.' in str(mc):
                op_file_name = 'Mstop_' + \
                    ...
                    (str(mc).replace('.', '_')) + '.pkl'

            elif '.' in str(ms) and '.' not in str(mc):
                op_file_name = 'Mstop_' + \
                    (str(ms).replace('.', '_')) + '_mchi_' + str(mc) + '.pkl'

            elif '.' not in str(ms) and '.' in str(mc):
                op_file_name = 'Mstop_' + \
                    str(ms) + '_mchi_' + (str(mc).replace('.', '_')) + '.pkl'

            else:
                op_file_name = 'Mstop_' + \
                    str(ms) + '_mchi_' + str(mc) + '.pkl'
            ...
            arr        = [h_ckk,   h_ckr,   h_crk,   h_cnn,   h_cnk,   h_ckn,   h_crr,   h_crn,   h_cnr, 
                        h_b1q,   h_b2q,   h_b1r,   h_b2r,   h_b1n,   h_b2n,   h_b1j,   h_b2j,     
                        h_ll_dphi,    h_ll_deta,   h_c_hel]

            df         = pd.DataFrame(data = arr).T

            df.columns = [ 'c_kk',  'c_kr',  'c_rk',  'c_nn',  'c_nk',  'c_kn',  'c_rr',  'c_rn',  'c_nr', 
                            'b1q',   'b2q',   'b1r',   'b2r',   'b1n',   'b2n',   'b1j',   'b2j', 
            ...
            df.to_pickle('Pkl_ips/SUSY_top_reco_op/' + str(op_file_name))
