...
hleppt_step0 = ROOT.TH1D('gen_lep_pt_0', 'gen_lep_pt_0', 18, 0, 180)

for i in range(len(step0['gen_lep_pt_0'].array())):
    hleppt_step0.Fill(step0['gen_lep_pt_0'].array()[i]) 

# Step 8 RECO hists

hleppt = ROOT.TH1D('lep_pt', 'lep_pt', 18, 0, 180)
hlepeta = ROOT.TH1D('lep_eta', 'lep_eta', 18, -7, 7)
hlepphi = ROOT.TH1D('lep_phi', 'lep_phi', 18, -np.pi, np.pi)

haleppt = ROOT.TH1D('alep_pt', 'alep_pt', 18, 0, 180)
halepeta = ROOT.TH1D('alep_eta', 'alep_eta', 18, -7, 7)
halepphi = ROOT.TH1D('alep_phi', 'alep_phi', 18, -np.pi, np.pi)

hmetpt = ROOT.TH1D('met_pt', 'met_pt', 18, 0, 180)
hmetphi = ROOT.TH1D('met_phi', 'met_phi', 18, -np.pi, np.pi)

hbpt = ROOT.TH1D('b_pt', 'b_pt', 18, 0, 180)
hbeta = ROOT.TH1D('b_eta', 'b_eta', 18, -7, 7)
hbphi = ROOT.TH1D('b_phi', 'b_phi', 18, -np.pi, np.pi)

habpt = ROOT.TH1D('ab_pt', 'ab_pt', 18, 0, 180)
habeta = ROOT.TH1D('ab_eta', 'ab_eta', 18, -7, 7)
habphi = ROOT.TH1D('ab_phi', 'ab_phi', 18, -np.pi, np.pi)

hneupt = ROOT.TH1D('neu_pt', 'neu_pt', 18, 0, 180)
hneueta = ROOT.TH1D('neu_eta', 'neu_eta', 18, -7, 7)
hneuphi = ROOT.TH1D('neu_phi', 'neu_phi', 18, -np.pi, np.pi)

haneupt = ROOT.TH1D('aneu_pt', 'aneu_pt', 18, 0, 180)
haneueta = ROOT.TH1D('aneu_eta', 'aneu_eta', 18, -7, 7)
haneuphi = ROOT.TH1D('aneu_phi', 'aneu_phi', 18, -np.pi, np.pi)

htpt = ROOT.TH1D('t_pt', 't_pt', 18, 0, 180)
hteta = ROOT.TH1D('t_eta', 't_eta', 18, -7, 7)
htphi = ROOT.TH1D('t_phi', 't_phi', 18, -np.pi, np.pi)
htrap = ROOT.TH1D("t_rap", "t_rap", 18, -7, 7)

hatpt = ROOT.TH1D('at_pt', 'at_pt', 18, 0, 180)
hateta = ROOT.TH1D('at_eta', 'at_eta', 18, -7, 7)
hatphi = ROOT.TH1D('at_phi', 'at_phi', 18, -np.pi, np.pi)
hatrap = ROOT.TH1D("at_rap", "at_rap", 18, -7, 7)

htatmass = ROOT.TH1D('tat_mass', 'tat_mass', 18, 340, 180)
htatpt = ROOT.TH1D("tat_pt", "tat_pt", 18, 0 ,180)
htateta = ROOT.TH1D("tat_eta", "tat_eta", 18, -7 ,7)
htatphi = ROOT.TH1D("tat_phi", "tat_phi", 18, -np.pi, np.pi)
htatrap = ROOT.TH1D("tat_rap", "tat_rap", 18, -7, 7)

hckk = ROOT.TH1D("ckk", "ckk", 18, -1, 1)
hcrr = ROOT.TH1D("crr", "crr", 18, -1, 1)
hcnn = ROOT.TH1D("cnn", "cnn", 18, -1, 1)

hcrk = ROOT.TH1D("crk", "crk", 18, -1, 1)
hckr = ROOT.TH1D("ckr", "ckr", 18, -1, 1)

hcPrk = ROOT.TH1D("cP_rk", "cP_rk", 18, -1, 1)
hcMrk = ROOT.TH1D("cM_rk", "cM_rk", 18, -1, 1)

hchel = ROOT.TH1D("c_hel", "c_hel", 18, -1, 1)

hlldeta = ROOT.TH1D("ll_deta", "ll_deta", 18, 0 ,7)
hlldphi = ROOT.TH1D("ll_dphi", "ll_dphi", 18, 0 ,np.pi)
hlldr = ROOT.TH1D("ll_dr", "ll_dr", 18, 0, 7)

# Step 8 GEN hists
hgenleppt = ROOT.TH1D('gen_lep_pt', 'gen_lep_pt', 18, 0, 180)
hgenlepeta = ROOT.TH1D('gen_lep_eta', 'gen_lep_eta', 18, -7, 7)
hgenlepphi = ROOT.TH1D('gen_lep_phi', 'gen_lep_phi', 18, -np.pi, np.pi)

hgenaleppt = ROOT.TH1D('gen_alep_pt', 'gen_alep_pt', 18, 0, 180)
hgenalepeta = ROOT.TH1D('gen_alep_eta', 'gen_alep_eta', 18, -7, 7)
hgenalepphi = ROOT.TH1D('gen_alep_phi', 'gen_alep_phi', 18, -np.pi, np.pi)

hgenmetpt = ROOT.TH1D('gen_met_pt', 'gen_met_pt', 18, 0, 180)
hgenmetphi = ROOT.TH1D('gen_met_phi', 'gen_met_phi', 18, -np.pi, np.pi)

hgenbpt = ROOT.TH1D('gen_b_pt', 'gen_b_pt', 18, 0, 180)
hgenbeta = ROOT.TH1D('gen_b_eta', 'gen_b_eta', 18, -7, 7)
hgenbphi = ROOT.TH1D('gen_b_phi', 'gen_b_phi', 18, -np.pi, np.pi)

hgenabpt = ROOT.TH1D('gen_ab_pt', 'gen_ab_pt', 18, 0, 180)
hgenabeta = ROOT.TH1D('gen_ab_eta', 'gen_ab_eta', 18, -7, 7)
hgenabphi = ROOT.TH1D('gen_ab_phi', 'gen_ab_phi', 18, -np.pi, np.pi)

hgenneupt = ROOT.TH1D('gen_neu_pt', 'gen_neu_pt', 18, 0, 180)
hgenneueta = ROOT.TH1D('gen_neu_eta', 'gen_neu_eta', 18, -7, 7)
hgenneuphi = ROOT.TH1D('gen_neu_phi', 'gen_neu_phi', 18, -np.pi, np.pi)

hgenaneupt = ROOT.TH1D('gen_aneu_pt', 'gen_aneu_pt', 18, 0, 180)
hgenaneueta = ROOT.TH1D('gen_aneu_eta', 'gen_aneu_eta', 18, -7, 7)
hgenaneuphi = ROOT.TH1D('gen_aneu_phi', 'gen_aneu_phi', 18, -np.pi, np.pi)

hgentpt = ROOT.TH1D('gen_t_pt', 'gen_t_pt', 18, 0, 180)
hgenteta = ROOT.TH1D('gen_t_eta', 'gen_t_eta', 18, -7, 7)
hgentphi = ROOT.TH1D('gen_t_phi', 'gen_t_phi', 18, -np.pi, np.pi)
hgentrap = ROOT.TH1D("gen_t_rap", "gen_t_rap", 18, -7, 7)

hgenatpt = ROOT.TH1D('gen_at_pt', 'gen_at_pt', 18, 0, 180)
hgenateta = ROOT.TH1D('gen_at_eta', 'gen_at_eta', 18, -7, 7)
hgenatphi = ROOT.TH1D('gen_at_phi', 'gen_at_phi', 18, -np.pi, np.pi)
hgenatrap = ROOT.TH1D("gen_at_rap", "gen_at_rap", 18, -7, 7)

hgentatmass = ROOT.TH1D('gen_tat_mass', 'gen_tat_mass', 18, 340, 180)
hgentatpt = ROOT.TH1D("gen_tat_pt", "gen_tat_pt", 18, 0 ,180)
hgentateta = ROOT.TH1D("gen_tat_eta", "gen_tat_eta", 18, -7 ,7)
hgentatphi = ROOT.TH1D("gen_tat_phi", "gen_tat_phi", 18, -np.pi, np.pi)
hgentatrap = ROOT.TH1D("gen_tat_rap", "gen_tat_rap", 18, -7, 7)

hgenckk = ROOT.TH1D("gen_ckk", "gen_ckk", 18, -1, 1)
hgencrr = ROOT.TH1D("gen_crr", "gen_crr", 18, -1, 1)
hgencnn = ROOT.TH1D("gen_cnn", "gen_cnn", 18, -1, 1)

hgencrk = ROOT.TH1D("gen_crk", "gen_crk", 18, -1, 1)
hgenckr = ROOT.TH1D("gen_ckr", "gen_ckr", 18, -1, 1)

hgencPrk = ROOT.TH1D("gen_cP_rk", "gen_cP_rk", 18, -1, 1)
hgencMrk = ROOT.TH1D("gen_cM_rk", "gen_cM_rk", 18, -1, 1)

hgenchel = ROOT.TH1D("gen_c_hel", "gen_c_hel", 18, -1, 1)

hgenlldeta = ROOT.TH1D("gen_ll_deta", "gen_ll_deta", 18, 0 ,7)
hgenlldphi = ROOT.TH1D("gen_ll_dphi", "gen_ll_dphi", 18, 0 ,np.pi)
hgenlldr = ROOT.TH1D("gen_ll_dr", "gen_ll_dr", 18, 0, 7)

#htestdphi = ROOT.TH1D("test_ll_dphi", "test_ll_dphi", 18, 0 ,np.pi)
#hgentestdphi = ROOT.TH1D("gen_test_ll_dphi", "gen_test_ll_dphi", 18, 0 ,np.pi)

#2D hists

hrvgleppt = ROOT.TH2D('rvg_lep_pt', 'reco vs gen lep pt', 18, 0, 180, 18, 0, 180)
hrvglepeta = ROOT.TH2D('rvg_lep_eta', 'reco vs gen lep eta', 18, -7, 7, 18, -7, 7)
hrvglepphi = ROOT.TH2D('rvg_lep_phi', 'reco vs gen lep phi', 18, -np.pi, np.pi, 18, -np.pi, np.pi)

hrvgaleppt = ROOT.TH2D('rvg_alep_pt', 'reco vs gen alep pt', 18, 0, 180, 18, 0, 180)
hrvgalepeta = ROOT.TH2D('rvg_alep_eta', 'reco vs gen alep eta', 18, -7, 7, 18, -7, 7)
hrvgalepphi = ROOT.TH2D('rvg_alep_phi', 'reco vs gen alep phi', 18, -np.pi, np.pi, 18, -np.pi, np.pi)

hrvgmetpt = ROOT.TH2D('rvg_met_pt', 'reco vs gen met pt', 18, 0, 180, 18, 0, 180)
hrvgmetphi = ROOT.TH2D('rvg_met_phi', 'reco vs gen met phi', 18, -np.pi, np.pi, 18, -np.pi, np.pi)

hrvgbpt = ROOT.TH2D('rvg_b_pt', 'reco vs gen b pt', 18, 0, 180, 18, 0, 180)
hrvgbeta = ROOT.TH2D('rvg_b_eta', 'reco vs gen b eta', 18, -7, 7, 18, -7, 7)
hrvgbphi = ROOT.TH2D('rvg_b_phi', 'reco vs gen b phi', 18, -np.pi, np.pi, 18, -np.pi, np.pi)

hrvgabpt = ROOT.TH2D('rvg_ab_pt', 'reco vs gen ab pt', 18, 0, 180, 18, 0, 180)
hrvgabeta = ROOT.TH2D('rvg_ab_eta', 'reco vs gen ab eta', 18, -7, 7, 18, -7, 7)
hrvgabphi = ROOT.TH2D('rvg_ab_phi', 'reco vs gen ab phi', 18, -np.pi, np.pi, 18, -np.pi, np.pi)

hrvgneupt = ROOT.TH2D('rvg_neu_pt', 'reco vs gen neu pt', 18, 0, 180, 18, 0, 180)
hrvgneueta = ROOT.TH2D('rvg_neu_eta', 'reco vs gen neu eta', 18, -7, 7, 18, -7, 7)
hrvgneuphi = ROOT.TH2D('rvg_neu_phi', 'reco vs gen neu phi', 18, -np.pi, np.pi, 18, -np.pi, np.pi)

hrvganeupt = ROOT.TH2D('rvg_aneu_pt', 'reco vs gen aneu pt', 18, 0, 180, 18, 0, 180)
hrvganeueta = ROOT.TH2D('rvg_aneu_eta', 'reco vs gen aneu eta', 18, -7, 7, 18, -7, 7)
hrvganeuphi = ROOT.TH2D('rvg_aneu_phi', 'reco vs gen aneu phi', 18, -np.pi, np.pi, 18, -np.pi, np.pi)

hrvgtpt = ROOT.TH2D('rvg_t_pt', 'reco vs gen t pt', 18, 0, 180, 18, 0, 180)
hrvgteta = ROOT.TH2D('rvg_t_eta', 'reco vs gen t eta', 18, -7, 7, 18, -7, 7)
hrvgtphi = ROOT.TH2D('rvg_t_phi', 'reco vs gen t phi', 18, -np.pi, np.pi, 18, -np.pi, np.pi)
hrvgtrap = ROOT.TH2D('rvg_t_rap', 'reco vs gen t rap', 18, -7, 7, 18, -7, 7)

hrvgatpt = ROOT.TH2D('rvg_at_pt', 'reco vs gen at pt', 18, 0, 180, 18, 0, 180)
hrvgateta = ROOT.TH2D('rvg_at_eta', 'reco vs gen at eta', 18, -7, 7, 18, -7, 7)
hrvgatphi = ROOT.TH2D('rvg_at_phi', 'reco vs gen at phi', 18, -np.pi, np.pi, 18, -np.pi, np.pi)
hrvgatrap = ROOT.TH2D('rvg_at_rap', 'reco vs gen at rap', 18, -7, 7, 18, -7, 7)

hrvgtatm = ROOT.TH2D('rvg_tat_m', "reco vs gen tat mass", 18, 300, 1400, 18, 300, 1400)
hrvgtatpt = ROOT.TH2D("rvg_tat_pt", "reco vs gen tat pt", 18, 0 ,180, 18, 0 ,180)
hrvgtateta = ROOT.TH2D("rvg_tat_eta", "reco vs gen tat eta", 18, -7 ,7, 18, -7 ,7)
hrvgtatphi = ROOT.TH2D("rvg_tat_phi", "reco vs gen tat phi", 18, -np.pi, np.pi, 18, -np.pi, np.pi)
hrvgtatrap = ROOT.TH2D("rvg_tat_rap", "reco vs gen tat rap", 18, -7, 7, 18, -7, 7)

hrvgckk = ROOT.TH2D("rvg_ckk", "reco vs gen ckk", 18, -1, 1, 18, -1, 1)
hrvgcrr = ROOT.TH2D("rvg_crr", "reco vs gen crr", 18, -1, 1, 18, -1, 1)
hrvgcnn = ROOT.TH2D("rvg_cnn", "reco vs gen cnn", 18, -1, 1, 18, -1, 1)

hrvgcrk = ROOT.TH2D('rvg_crk', "reco vs gen crk", 18, -1, 1, 18, -1, 1)
hrvgckr = ROOT.TH2D('rvg_ckr', "reco vs gen ckr", 18, -1, 1, 18, -1, 1)

hrvgcPrk = ROOT.TH2D("rvg_cPrk", "reco vs gen cPrk", 18, -1, 1, 18, -1, 1)
hrvgcMrk = ROOT.TH2D("rvg_cMrk", "reco vs gen cMrk", 18, -1, 1, 18, -1, 1)

hrvgchel = ROOT.TH2D("rvg_c_hel", "reco vs gen c_hel", 18, -1, 1, 18, -1, 1)

hrvglldeta = ROOT.TH2D("rvg_ll_deta", "reco vs gen ll deta", 18, 0 ,7, 18, 0 ,7)
hrvglldphi = ROOT.TH2D("rvg_ll_dphi", "reco vs gen ll dphi", 18, 0 ,np.pi, 18, 0 ,np.pi)
hrvglldr = ROOT.TH2D("rvg_ll_dr", "reco vs gen ll dr", 18, 0, 7, 18, 0, 7)
