...
step0_haleppt = ROOT.TH1D('all_gen_alep_pt', 'all_gen_alep_pt', 30, 0, 300)
...
step0_hmetpt = ROOT.TH1D('all_gen_met_pt', 'all_gen_met_pt', 30, 0, 300)
step0_hmetphi = ROOT.TH1D('all_gen_met_phi', 'all_gen_met_phi', 30, -np.pi, np.pi)
step0_htpt = ROOT.TH1D('all_gen_top_pt', 'all_gen_top_pt', 30, 0, 600)
step0_hteta = ROOT.TH1D('all_gen_top_eta', 'all_gen_top_eta', 30, -7, 7)
step0_htphi = ROOT.TH1D('all_gen_top_phi', 'all_gen_top_phi', 30, -np.pi, np.pi)
step0_hatpt = ROOT.TH1D('all_gen_atop_pt', 'all_gen_atop_pt', 30, 0, 600)
step0_hateta = ROOT.TH1D('all_gen_atop_eta', 'all_gen_atop_eta', 30, -7, 7)
step0_hatphi = ROOT.TH1D('all_gen_atop_phi', 'all_gen_atop_phi', 30, -np.pi, np.pi)
step0_hbpt = ROOT.TH1D('all_gen_b_pt', 'all_gen_b_pt', 30, 0, 300)
step0_hbeta = ROOT.TH1D('all_gen_b_eta', 'all_gen_b_eta', 30, -7, 7)
step0_hbphi = ROOT.TH1D('all_gen_b_phi', 'all_gen_b_phi', 30, -np.pi, np.pi)
step0_habpt = ROOT.TH1D('all_gen_ab_pt', 'all_gen_ab_pt', 30, 0, 300)
...
step0_hneupt = ROOT.TH1D('all_gen_neu_pt', 'all_gen_neu_pt', 30, 0, 300)
...
step0_haneupt = ROOT.TH1D('all_gen_aneu_pt', 'all_gen_aneu_pt', 30, 0, 300)
...
hleppt = ROOT.TH1D('lep_pt', 'lep_pt', 30, 0, 300)
hlepeta = ROOT.TH1D('lep_eta', 'lep_eta', 30, -7, 7)
hlepphi = ROOT.TH1D('lep_phi', 'lep_phi', 30, -np.pi, np.pi)

haleppt = ROOT.TH1D('alep_pt', 'alep_pt', 30, 0, 300)
halepeta = ROOT.TH1D('alep_eta', 'alep_eta', 30, -7, 7)
halepphi = ROOT.TH1D('alep_phi', 'alep_phi', 30, -np.pi, np.pi)

hmetpt = ROOT.TH1D('met_pt', 'met_pt', 30, 0, 300)
hmetphi = ROOT.TH1D('met_phi', 'met_phi', 30, -np.pi, np.pi)

hbpt = ROOT.TH1D('b_pt', 'b_pt', 30, 0, 300)
hbeta = ROOT.TH1D('b_eta', 'b_eta', 30, -7, 7)
hbphi = ROOT.TH1D('b_phi', 'b_phi', 30, -np.pi, np.pi)

habpt = ROOT.TH1D('ab_pt', 'ab_pt', 30, 0, 300)
habeta = ROOT.TH1D('ab_eta', 'ab_eta', 30, -7, 7)
habphi = ROOT.TH1D('ab_phi', 'ab_phi', 30, -np.pi, np.pi)

hneupt = ROOT.TH1D('neu_pt', 'neu_pt', 30, 0, 300)
hneueta = ROOT.TH1D('neu_eta', 'neu_eta', 30, -7, 7)
hneuphi = ROOT.TH1D('neu_phi', 'neu_phi', 30, -np.pi, np.pi)

haneupt = ROOT.TH1D('aneu_pt', 'aneu_pt', 30, 0, 300)
haneueta = ROOT.TH1D('aneu_eta', 'aneu_eta', 30, -7, 7)
haneuphi = ROOT.TH1D('aneu_phi', 'aneu_phi', 30, -np.pi, np.pi)

htpt = ROOT.TH1D('top_pt', 'top_pt', 30, 0, 600)
hteta = ROOT.TH1D('top_eta', 'top_eta', 30, -7, 7)
htphi = ROOT.TH1D('top_phi', 'top_phi', 30, -np.pi, np.pi)
htrap = ROOT.TH1D("top_rap", "top_rap", 30, -7, 7)

hatpt = ROOT.TH1D('atop_pt', 'atop_pt', 30, 0, 600)
hateta = ROOT.TH1D('atop_eta', 'atop_eta', 30, -7, 7)
hatphi = ROOT.TH1D('atop_phi', 'atop_phi', 30, -np.pi, np.pi)
hatrap = ROOT.TH1D("atop_rap", "atop_rap", 30, -7, 7)

htatmass = ROOT.TH1D('tat_mass', 'tat_mass', 30, 0, 2400)
htatpt = ROOT.TH1D("tat_pt", "tat_pt", 30, 0 , 300)
...
# Step 8 GEN hists
hgenleppt = ROOT.TH1D('gen_lep_pt', 'gen_lep_pt', 30, 0, 300)
hgenlepeta = ROOT.TH1D('gen_lep_eta', 'gen_lep_eta', 30, -7, 7)
hgenlepphi = ROOT.TH1D('gen_lep_phi', 'gen_lep_phi', 30, -np.pi, np.pi)

hgenaleppt = ROOT.TH1D('gen_alep_pt', 'gen_alep_pt', 30, 0, 300)
hgenalepeta = ROOT.TH1D('gen_alep_eta', 'gen_alep_eta', 30, -7, 7)
hgenalepphi = ROOT.TH1D('gen_alep_phi', 'gen_alep_phi', 30, -np.pi, np.pi)

hgenmetpt = ROOT.TH1D('gen_met_pt', 'gen_met_pt', 30, 0, 300)
hgenmetphi = ROOT.TH1D('gen_met_phi', 'gen_met_phi', 30, -np.pi, np.pi)

hgenbpt = ROOT.TH1D('gen_b_pt', 'gen_b_pt', 30, 0, 300)
hgenbeta = ROOT.TH1D('gen_b_eta', 'gen_b_eta', 30, -7, 7)
hgenbphi = ROOT.TH1D('gen_b_phi', 'gen_b_phi', 30, -np.pi, np.pi)

hgenabpt = ROOT.TH1D('gen_ab_pt', 'gen_ab_pt', 30, 0, 300)
hgenabeta = ROOT.TH1D('gen_ab_eta', 'gen_ab_eta', 30, -7, 7)
hgenabphi = ROOT.TH1D('gen_ab_phi', 'gen_ab_phi', 30, -np.pi, np.pi)

hgenneupt = ROOT.TH1D('gen_neu_pt', 'gen_neu_pt', 30, 0, 300)
hgenneueta = ROOT.TH1D('gen_neu_eta', 'gen_neu_eta', 30, -7, 7)
hgenneuphi = ROOT.TH1D('gen_neu_phi', 'gen_neu_phi', 30, -np.pi, np.pi)

hgenaneupt = ROOT.TH1D('gen_aneu_pt', 'gen_aneu_pt', 30, 0, 300)
hgenaneueta = ROOT.TH1D('gen_aneu_eta', 'gen_aneu_eta', 30, -7, 7)
hgenaneuphi = ROOT.TH1D('gen_aneu_phi', 'gen_aneu_phi', 30, -np.pi, np.pi)

hgentpt = ROOT.TH1D('gen_top_pt', 'gen_top_pt', 30, 0, 600)
hgenteta = ROOT.TH1D('gen_top_eta', 'gen_top_eta', 30, -7, 7)
hgentphi = ROOT.TH1D('gen_top_phi', 'gen_top_phi', 30, -np.pi, np.pi)
hgentrap = ROOT.TH1D("gen_top_rap", "gen_top_rap", 30, -7, 7)

hgenatpt = ROOT.TH1D('gen_atop_pt', 'gen_atop_pt', 30, 0, 600)
hgenateta = ROOT.TH1D('gen_atop_eta', 'gen_atop_eta', 30, -7, 7)
hgenatphi = ROOT.TH1D('gen_atop_phi', 'gen_atop_phi', 30, -np.pi, np.pi)
hgenatrap = ROOT.TH1D("gen_atop_rap", "gen_atop_rap", 30, -7, 7)

hgentatmass = ROOT.TH1D('gen_tat_mass', 'gen_tat_mass', 30, 0, 2400)
hgentatpt = ROOT.TH1D("gen_tat_pt", "gen_tat_pt", 30, 0 , 600)
...
hrvgleppt = ROOT.TH2D('rvg_lep_pt', 'reco vs gen lep pt', 30, 0, 300, 30, 0, 300)
...
hrvgaleppt = ROOT.TH2D('rvg_alep_pt', 'reco vs gen alep pt', 30, 0, 300, 30, 0, 300)
...
hrvgmetpt = ROOT.TH2D('rvg_met_pt', 'reco vs gen met pt', 30, 0, 300, 30, 0, 300)
hrvgmetphi = ROOT.TH2D('rvg_met_phi', 'reco vs gen met phi', 30, -np.pi, np.pi, 30, -np.pi, np.pi)

hrvgbpt = ROOT.TH2D('rvg_b_pt', 'reco vs gen b pt', 30, 0, 300, 30, 0, 300)
...
hrvgabpt = ROOT.TH2D('rvg_ab_pt', 'reco vs gen ab pt', 30, 0, 300, 30, 0, 300)
...
hrvgneupt = ROOT.TH2D('rvg_neu_pt', 'reco vs gen neu pt', 30, 0, 300, 30, 0, 300)
...
hrvganeupt = ROOT.TH2D('rvg_aneu_pt', 'reco vs gen aneu pt', 30, 0, 300, 30, 0, 300)
...
hrvgtpt = ROOT.TH2D('rvg_top_pt', 'reco vs gen t pt', 30, 0, 600, 30, 0, 600)
hrvgteta = ROOT.TH2D('rvg_top_eta', 'reco vs gen t eta', 30, -7, 7, 30, -7, 7)
hrvgtphi = ROOT.TH2D('rvg_top_phi', 'reco vs gen t phi', 30, -np.pi, np.pi, 30, -np.pi, np.pi)
hrvgtrap = ROOT.TH2D('rvg_top_rap', 'reco vs gen t rap', 30, -7, 7, 30, -7, 7) 

hrvgatpt = ROOT.TH2D('rvg_atop_pt', 'reco vs gen at pt', 30, 0, 600, 30, 0, 600)
hrvgateta = ROOT.TH2D('rvg_atop_eta', 'reco vs gen at eta', 30, -7, 7, 30, -7, 7)
hrvgatphi = ROOT.TH2D('rvg_atop_phi', 'reco vs gen at phi', 30, -np.pi, np.pi, 30, -np.pi, np.pi)
hrvgatrap = ROOT.TH2D('rvg_atop_rap', 'reco vs gen at rap', 30, -7, 7, 30, -7, 7)

hrvgtatmass = ROOT.TH2D('rvg_tat_mass', "reco vs gen tat mass", 30, 0, 2400, 30, 0, 2400)
hrvgtatpt = ROOT.TH2D("rvg_tat_pt", "reco vs gen tat pt", 30, 0, 300, 30, 0, 300)
...
for i in range(len(t_pt)):
    ...
    hgentrap.Fill(fileptr['gen_top_rap'].array()[i])
    ...
    hgenatrap.Fill(fileptr['gen_atop_rap'].array()[i])
    ...
    hrvgneupt.Fill(fileptr['neu_pt'].array()[i], fileptr['gen_neu_pt'].array()[i])
    hrvgneueta.Fill(fileptr['neu_eta'].array()[i], fileptr['gen_neu_eta'].array()[i])
    hrvgneuphi.Fill(fileptr['neu_phi'].array()[i], fileptr['gen_neu_phi'].array()[i])

    hrvganeupt.Fill(fileptr['aneu_pt'].array()[i], fileptr['gen_aneu_pt'].array()[i])
    hrvganeueta.Fill(fileptr['aneu_eta'].array()[i], fileptr['gen_aneu_eta'].array()[i])
    hrvganeuphi.Fill(fileptr['aneu_phi'].array()[i], fileptr['gen_aneu_phi'].array()[i])    
    ...
    hrvgtrap.Fill(fileptr['top_rap'].array()[i], fileptr['gen_top_rap'].array()[i])
    ...
    hrvgatrap.Fill(fileptr['atop_rap'].array()[i], fileptr['gen_atop_rap'].array()[i])

    hrvgtatmass.Fill(tt_mass[i], fileptr['gen_tt_mass'].array()[i])
