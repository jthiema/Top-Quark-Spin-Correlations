...
step0_hleppt = ROOT.TH1D('all_gen_lep_pt', 'all_gen_lep_pt', 1200, 0, 1200)
step0_hlepeta = ROOT.TH1D('all_gen_lep_eta', 'all_gen_lep_eta', 1200, -7, 7)
step0_hlepphi = ROOT.TH1D('all_gen_lep_phi', 'all_gen_lep_phi', 1200, -np.pi, np.pi)
step0_haleppt = ROOT.TH1D('all_gen_alep_pt', 'all_gen_alep_pt', 1200, 0, 1200)
step0_halepeta = ROOT.TH1D('all_gen_alep_eta', 'all_gen_alep_eta', 1200, -7, 7)
step0_halepphi = ROOT.TH1D('all_gen_alep_phi', 'all_gen_alep_phi', 1200, -np.pi, np.pi)
step0_hmetpt = ROOT.TH1D('all_gen_met_pt', 'all_gen_met_pt', 1200, 0, 1200)
step0_hmetphi = ROOT.TH1D('all_gen_met_phi', 'all_gen_met_phi', 1200, -np.pi, np.pi)
step0_htpt = ROOT.TH1D('all_gen_t_pt', 'all_gen_t_pt', 1200, 0, 1200)
step0_hteta = ROOT.TH1D('all_gen_t_eta', 'all_gen_t_eta', 1200, -7, 7)
step0_htphi = ROOT.TH1D('all_gen_t_phi', 'all_gen_t_phi', 1200, -np.pi, np.pi)
step0_hatpt = ROOT.TH1D('all_gen_at_pt', 'all_gen_at_pt', 1200, 0, 1200)
step0_hateta = ROOT.TH1D('all_gen_at_eta', 'all_gen_at_eta', 1200, -7, 7)
step0_hatphi = ROOT.TH1D('all_gen_at_phi', 'all_gen_at_phi', 1200, -np.pi, np.pi)
step0_hbpt = ROOT.TH1D('all_gen_b_pt', 'all_gen_b_pt', 1200, 0, 1200)
step0_hbeta = ROOT.TH1D('all_gen_b_eta', 'all_gen_b_eta', 1200, -7, 7)
step0_hbphi = ROOT.TH1D('all_gen_b_phi', 'all_gen_b_phi', 1200, -np.pi, np.pi)
step0_habpt = ROOT.TH1D('all_gen_ab_pt', 'all_gen_ab_pt', 1200, 0, 1200)
step0_habeta = ROOT.TH1D('all_gen_ab_eta', 'all_gen_ab_eta', 1200, -7, 7)
step0_habphi = ROOT.TH1D('all_gen_ab_phi', 'all_gen_ab_phi', 1200, -np.pi, np.pi)
step0_hneupt = ROOT.TH1D('all_gen_neu_pt', 'all_gen_neu_pt', 1200, 0, 1200)
step0_hneueta = ROOT.TH1D('all_gen_neu_eta', 'all_gen_neu_eta', 1200, -7, 7)
step0_hneuphi = ROOT.TH1D('all_gen_neu_phi', 'all_gen_neu_phi', 1200, -np.pi, np.pi)
step0_haneupt = ROOT.TH1D('all_gen_aneu_pt', 'all_gen_aneu_pt', 1200, 0, 1200)
step0_haneueta = ROOT.TH1D('all_gen_aneu_eta', 'all_gen_aneu_eta', 1200, -7, 7)
step0_haneuphi = ROOT.TH1D('all_gen_aneu_phi', 'all_gen_aneu_phi', 1200, -np.pi, np.pi)
...
hlepeta = ROOT.TH1D('lep_eta', 'lep_eta', 1200, -7, 7)
hlepphi = ROOT.TH1D('lep_phi', 'lep_phi', 1200, -np.pi, np.pi)

haleppt = ROOT.TH1D('alep_pt', 'alep_pt', 1200, 0, 1200)
halepeta = ROOT.TH1D('alep_eta', 'alep_eta', 1200, -7, 7)
halepphi = ROOT.TH1D('alep_phi', 'alep_phi', 1200, -np.pi, np.pi)

hmetpt = ROOT.TH1D('met_pt', 'met_pt', 1200, 0, 1200)
hmetphi = ROOT.TH1D('met_phi', 'met_phi', 1200, -np.pi, np.pi)

hbpt = ROOT.TH1D('b_pt', 'b_pt', 1200, 0, 1200)
hbeta = ROOT.TH1D('b_eta', 'b_eta', 1200, -7, 7)
hbphi = ROOT.TH1D('b_phi', 'b_phi', 1200, -np.pi, np.pi)

habpt = ROOT.TH1D('ab_pt', 'ab_pt', 1200, 0, 1200)
habeta = ROOT.TH1D('ab_eta', 'ab_eta', 1200, -7, 7)
habphi = ROOT.TH1D('ab_phi', 'ab_phi', 1200, -np.pi, np.pi)

hneupt = ROOT.TH1D('neu_pt', 'neu_pt', 1200, 0, 1200)
hneueta = ROOT.TH1D('neu_eta', 'neu_eta', 1200, -7, 7)
hneuphi = ROOT.TH1D('neu_phi', 'neu_phi', 1200, -np.pi, np.pi)

haneupt = ROOT.TH1D('aneu_pt', 'aneu_pt', 1200, 0, 1200)
haneueta = ROOT.TH1D('aneu_eta', 'aneu_eta', 1200, -7, 7)
haneuphi = ROOT.TH1D('aneu_phi', 'aneu_phi', 1200, -np.pi, np.pi)

htpt = ROOT.TH1D('t_pt', 't_pt', 1200, 0, 1200)
hteta = ROOT.TH1D('t_eta', 't_eta', 1200, -7, 7)
htphi = ROOT.TH1D('t_phi', 't_phi', 1200, -np.pi, np.pi)
htrap = ROOT.TH1D("t_rap", "t_rap", 1200, -7, 7)

hatpt = ROOT.TH1D('at_pt', 'at_pt', 1200, 0, 1200)
hateta = ROOT.TH1D('at_eta', 'at_eta', 1200, -7, 7)
hatphi = ROOT.TH1D('at_phi', 'at_phi', 1200, -np.pi, np.pi)
hatrap = ROOT.TH1D("at_rap", "at_rap", 1200, -7, 7)

htatmass = ROOT.TH1D('tat_mass', 'tat_mass', 1200, 0, 1200)
htatpt = ROOT.TH1D("tat_pt", "tat_pt", 1200, 0 ,1200)
htateta = ROOT.TH1D("tat_eta", "tat_eta", 1200, -7 ,7)
htatphi = ROOT.TH1D("tat_phi", "tat_phi", 1200, -np.pi, np.pi)
htatrap = ROOT.TH1D("tat_rap", "tat_rap", 1200, -7, 7)
...
hlldeta = ROOT.TH1D("ll_deta", "ll_deta", 1200, 0 ,7)
hlldphi = ROOT.TH1D("ll_dphi", "ll_dphi", 1200, 0 ,np.pi)
hlldr = ROOT.TH1D("ll_dr", "ll_dr", 1200, 0, 7)

# Step 8 GEN hists
hgenleppt = ROOT.TH1D('gen_lep_pt', 'gen_lep_pt', 1200, 0, 1200)
hgenlepeta = ROOT.TH1D('gen_lep_eta', 'gen_lep_eta', 1200, -7, 7)
hgenlepphi = ROOT.TH1D('gen_lep_phi', 'gen_lep_phi', 1200, -np.pi, np.pi)

hgenaleppt = ROOT.TH1D('gen_alep_pt', 'gen_alep_pt', 1200, 0, 1200)
hgenalepeta = ROOT.TH1D('gen_alep_eta', 'gen_alep_eta', 1200, -7, 7)
hgenalepphi = ROOT.TH1D('gen_alep_phi', 'gen_alep_phi', 1200, -np.pi, np.pi)

hgenmetpt = ROOT.TH1D('gen_met_pt', 'gen_met_pt', 1200, 0, 1200)
hgenmetphi = ROOT.TH1D('gen_met_phi', 'gen_met_phi', 1200, -np.pi, np.pi)

hgenbpt = ROOT.TH1D('gen_b_pt', 'gen_b_pt', 1200, 0, 1200)
hgenbeta = ROOT.TH1D('gen_b_eta', 'gen_b_eta', 1200, -7, 7)
hgenbphi = ROOT.TH1D('gen_b_phi', 'gen_b_phi', 1200, -np.pi, np.pi)

hgenabpt = ROOT.TH1D('gen_ab_pt', 'gen_ab_pt', 1200, 0, 1200)
hgenabeta = ROOT.TH1D('gen_ab_eta', 'gen_ab_eta', 1200, -7, 7)
hgenabphi = ROOT.TH1D('gen_ab_phi', 'gen_ab_phi', 1200, -np.pi, np.pi)

hgenneupt = ROOT.TH1D('gen_neu_pt', 'gen_neu_pt', 1200, 0, 1200)
hgenneueta = ROOT.TH1D('gen_neu_eta', 'gen_neu_eta', 1200, -7, 7)
hgenneuphi = ROOT.TH1D('gen_neu_phi', 'gen_neu_phi', 1200, -np.pi, np.pi)

hgenaneupt = ROOT.TH1D('gen_aneu_pt', 'gen_aneu_pt', 1200, 0, 1200)
hgenaneueta = ROOT.TH1D('gen_aneu_eta', 'gen_aneu_eta', 1200, -7, 7)
hgenaneuphi = ROOT.TH1D('gen_aneu_phi', 'gen_aneu_phi', 1200, -np.pi, np.pi)

hgentpt = ROOT.TH1D('gen_t_pt', 'gen_t_pt', 1200, 0, 1200)
hgenteta = ROOT.TH1D('gen_t_eta', 'gen_t_eta', 1200, -7, 7)
hgentphi = ROOT.TH1D('gen_t_phi', 'gen_t_phi', 1200, -np.pi, np.pi)
hgentrap = ROOT.TH1D("gen_t_rap", "gen_t_rap", 1200, -7, 7)

hgenatpt = ROOT.TH1D('gen_at_pt', 'gen_at_pt', 1200, 0, 1200)
hgenateta = ROOT.TH1D('gen_at_eta', 'gen_at_eta', 1200, -7, 7)
hgenatphi = ROOT.TH1D('gen_at_phi', 'gen_at_phi', 1200, -np.pi, np.pi)
hgenatrap = ROOT.TH1D("gen_at_rap", "gen_at_rap", 1200, -7, 7)

hgentatmass = ROOT.TH1D('gen_tat_mass', 'gen_tat_mass', 1200, 0, 1200)
hgentatpt = ROOT.TH1D("gen_tat_pt", "gen_tat_pt", 1200, 0 ,1200)
hgentateta = ROOT.TH1D("gen_tat_eta", "gen_tat_eta", 1200, -7 ,7)
hgentatphi = ROOT.TH1D("gen_tat_phi", "gen_tat_phi", 1200, -np.pi, np.pi)
hgentatrap = ROOT.TH1D("gen_tat_rap", "gen_tat_rap", 1200, -7, 7)
...
hgenlldeta = ROOT.TH1D("gen_ll_deta", "gen_ll_deta", 1200, 0 ,7)
hgenlldphi = ROOT.TH1D("gen_ll_dphi", "gen_ll_dphi", 1200, 0 ,np.pi)
hgenlldr = ROOT.TH1D("gen_ll_dr", "gen_ll_dr", 1200, 0, 7)

#htestdphi = ROOT.TH1D("test_ll_dphi", "test_ll_dphi", 1200, 0 ,np.pi)
#hgentestdphi = ROOT.TH1D("gen_test_ll_dphi", "gen_test_ll_dphi", 1200, 0 ,np.pi)

#2D hists

hrvgleppt = ROOT.TH2D('rvg_lep_pt', 'reco vs gen lep pt', 1200, 0, 1200, 1200, 0, 1200)
hrvglepeta = ROOT.TH2D('rvg_lep_eta', 'reco vs gen lep eta', 1200, -7, 7, 1200, -7, 7)
hrvglepphi = ROOT.TH2D('rvg_lep_phi', 'reco vs gen lep phi', 1200, -np.pi, np.pi, 1200, -np.pi, np.pi)

hrvgaleppt = ROOT.TH2D('rvg_alep_pt', 'reco vs gen alep pt', 1200, 0, 1200, 1200, 0, 1200)
hrvgalepeta = ROOT.TH2D('rvg_alep_eta', 'reco vs gen alep eta', 1200, -7, 7, 1200, -7, 7)
hrvgalepphi = ROOT.TH2D('rvg_alep_phi', 'reco vs gen alep phi', 1200, -np.pi, np.pi, 1200, -np.pi, np.pi)

hrvgmetpt = ROOT.TH2D('rvg_met_pt', 'reco vs gen met pt', 1200, 0, 1200, 1200, 0, 1200)
hrvgmetphi = ROOT.TH2D('rvg_met_phi', 'reco vs gen met phi', 1200, -np.pi, np.pi, 1200, -np.pi, np.pi)

hrvgbpt = ROOT.TH2D('rvg_b_pt', 'reco vs gen b pt', 1200, 0, 1200, 1200, 0, 1200)
hrvgbeta = ROOT.TH2D('rvg_b_eta', 'reco vs gen b eta', 1200, -7, 7, 1200, -7, 7)
hrvgbphi = ROOT.TH2D('rvg_b_phi', 'reco vs gen b phi', 1200, -np.pi, np.pi, 1200, -np.pi, np.pi)

hrvgabpt = ROOT.TH2D('rvg_ab_pt', 'reco vs gen ab pt', 1200, 0, 1200, 1200, 0, 1200)
hrvgabeta = ROOT.TH2D('rvg_ab_eta', 'reco vs gen ab eta', 1200, -7, 7, 1200, -7, 7)
hrvgabphi = ROOT.TH2D('rvg_ab_phi', 'reco vs gen ab phi', 1200, -np.pi, np.pi, 1200, -np.pi, np.pi)

hrvgneupt = ROOT.TH2D('rvg_neu_pt', 'reco vs gen neu pt', 1200, 0, 1200, 1200, 0, 1200)
hrvgneueta = ROOT.TH2D('rvg_neu_eta', 'reco vs gen neu eta', 1200, -7, 7, 1200, -7, 7)
hrvgneuphi = ROOT.TH2D('rvg_neu_phi', 'reco vs gen neu phi', 1200, -np.pi, np.pi, 1200, -np.pi, np.pi)

hrvganeupt = ROOT.TH2D('rvg_aneu_pt', 'reco vs gen aneu pt', 1200, 0, 1200, 1200, 0, 1200)
hrvganeueta = ROOT.TH2D('rvg_aneu_eta', 'reco vs gen aneu eta', 1200, -7, 7, 1200, -7, 7)
hrvganeuphi = ROOT.TH2D('rvg_aneu_phi', 'reco vs gen aneu phi', 1200, -np.pi, np.pi, 1200, -np.pi, np.pi)

hrvgtpt = ROOT.TH2D('rvg_t_pt', 'reco vs gen t pt', 1200, 0, 1200, 1200, 0, 1200)
hrvgteta = ROOT.TH2D('rvg_t_eta', 'reco vs gen t eta', 1200, -7, 7, 1200, -7, 7)
hrvgtphi = ROOT.TH2D('rvg_t_phi', 'reco vs gen t phi', 1200, -np.pi, np.pi, 1200, -np.pi, np.pi)
hrvgtrap = ROOT.TH2D('rvg_t_rap', 'reco vs gen t rap', 1200, -7, 7, 1200, -7, 7) 

hrvgatpt = ROOT.TH2D('rvg_at_pt', 'reco vs gen at pt', 1200, 0, 1200, 1200, 0, 1200)
hrvgateta = ROOT.TH2D('rvg_at_eta', 'reco vs gen at eta', 1200, -7, 7, 1200, -7, 7)
hrvgatphi = ROOT.TH2D('rvg_at_phi', 'reco vs gen at phi', 1200, -np.pi, np.pi, 1200, -np.pi, np.pi)
hrvgatrap = ROOT.TH2D('rvg_at_rap', 'reco vs gen at rap', 1200, -7, 7, 1200, -7, 7)

hrvgtatm = ROOT.TH2D('rvg_tat_m', "reco vs gen tat mass", 1200, 300, 1200, 1200, 300, 1400)
hrvgtatpt = ROOT.TH2D("rvg_tat_pt", "reco vs gen tat pt", 1200, 0 ,1200, 1200, 0 ,1200)
hrvgtateta = ROOT.TH2D("rvg_tat_eta", "reco vs gen tat eta", 1200, -7 ,7, 1200, -7 ,7)
hrvgtatphi = ROOT.TH2D("rvg_tat_phi", "reco vs gen tat phi", 1200, -np.pi, np.pi, 1200, -np.pi, np.pi)
hrvgtatrap = ROOT.TH2D("rvg_tat_rap", "reco vs gen tat rap", 1200, -7, 7, 1200, -7, 7)
...
hrvglldeta = ROOT.TH2D("rvg_ll_deta", "reco vs gen ll deta", 1200, 0 ,7, 1200, 0 ,7)
hrvglldphi = ROOT.TH2D("rvg_ll_dphi", "reco vs gen ll dphi", 1200, 0 ,np.pi, 1200, 0 ,np.pi)
hrvglldr = ROOT.TH2D("rvg_ll_dr", "reco vs gen ll dr", 1200, 0, 7, 1200, 0, 7)
